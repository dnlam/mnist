...
def compute_accuracy_gpu(predictions, y):
    """Computes the accuracy of predictions against the gold labels, y."""
    return torch.tensor(torch.sum(predictions==y).item()/len(predictions))
...
def run_epoch(data, model, optimizer):
    ...
    for batch in tqdm(data):
        ...
        batch_accuracies_first.append(compute_accuracy_gpu(predictions_first_label, y[0]))
        batch_accuracies_second.append(compute_accuracy_gpu(predictions_second_label, y[1]))
